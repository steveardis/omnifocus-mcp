...
- `hasDeferDate` (literal `true`): return only tasks where `deferDate` is non-null
...
#### Scenario: Filter by hasDeferDate
- **WHEN** `list_tasks` is called with `{ scope: { all: true }, filter: { hasDeferDate: true } }`
- **THEN** the tool returns only tasks where `deferDate` is non-null, excluding complete and dropped tasks
...
The `TaskSummary` returned by `list_tasks` SHALL include `dueDate` (ISO datetime or null), `deferDate` (ISO datetime or null), `plannedDate` (ISO datetime or null), and `tagIds` (array of tag ID strings) in addition to existing fields.
...
#### Scenario: Summary includes deferDate
- **WHEN** `list_tasks` returns a task that has a defer date set
- **THEN** the task summary includes `deferDate` as an ISO datetime string

#### Scenario: Summary includes plannedDate
- **WHEN** `list_tasks` returns a task that has a planned date set
- **THEN** the task summary includes `plannedDate` as an ISO datetime string
