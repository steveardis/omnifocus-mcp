...
#### Scenario: Task summary includes deferDate and plannedDate
- **WHEN** `list_tasks` returns a task that has both a defer date and a planned date set
- **THEN** the task summary includes both `deferDate` and `plannedDate` as ISO datetime strings
